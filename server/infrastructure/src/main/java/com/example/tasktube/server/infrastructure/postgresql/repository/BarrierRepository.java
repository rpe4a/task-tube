...
public class BarrierRepository implements IBarrierRepository {
    ...
    @Override
    public Optional<Barrier> getById(final UUID barrierId) {
    ...
    @Override
    public List<Barrier> getByTaskId(final UUID taskId) {
        if (Objects.isNull(taskId)) {
            throw new ApplicationException("Parameter taskId cannot be null.");
        }
        LOGGER.debug("Attempting to get barriers by task ID: '{}'.", taskId);
        ...
        return db.query(queryCommand, Map.of("task_id", taskId), rsMapper);
    }
